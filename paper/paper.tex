...
\begin{algorithm}[H]
    ...
    \For{$i = 1, \ldots, m$}{
        ...
        $\hat{H} = \text{LinearRegression}(X_2, Y_2 \hat{G})$\;
        $E \leftarrow E + \text{Diag}(H)$\;
    }
    \Return{$\hat{E} / m$}
...
\subsection{Post-processing of B2BR}
...
%
Letting $m_0$ and $m_1$ be the average values of the two clusters, $p$ and $d_x-p$ their size, and $v$ the total variance of the sample, we select the split maximizing the Sonquist-Morgan criterion $\frac{p(d_x-p)}{d_x} \frac{(m_1 - m_0)^2}{v}$.
%
