...
\title{Discriminating causes \\from multivariate observations:\\the back-to-back regression}
...
\affil[1]{Laboratoire des systèmes perceptifs, PSL University, CNRS}
\affil[2]{Facebook AI}
\date{}
...
observations are complex and multi-dimensional. To address this issue, we
introduce ``Back-to-Back'' regression (B2B), a method designed to efficiently
measure, from a set of colinear factors, those that most plausibly account for
multidimensional observations. After proving the consistency of B2B and its
links to other linear approaches, we show that our method outperforms
least-squares regression and cross-decomposition techniques (e.g. canonical
correlation analysis and partial least squares) on causal identification.
Finally, we apply B2B to neuroimaging recordings of 102 subjects reading word
sequences. The results show that the early and late brain representations,
caused by low- and high-level word features respectively, are more reliably
detected with B2B than with other standard techniques.
...
d_y}$, generated from a set of putative causes $X \in \mathbb{R}^{n \times
d_x}$, via some unknown linear apparatus $F \in \mathbb{R}^{d_x \times d_y}$.
%
...
\end{equation}
%
where $i$ is a given sample, and $n_i$ is a sample-specific noise drawen from a
...
%
Given the model in Equation~\eqref{eq:model}, \textbf{the goal} of Back-to-Back
Regression (B2B) is to estimate the matrix of causal influences $E$.
...
http://cbcl.mit.edu/publications/ps/MIT-CSAIL-TR-2007-025.pdf, as follows:
%
...
\begin{theorem}[B2B consistency - general case]
     ...
     Consider the B2B model from Equation $Y = (XE + N)F$, $N$ centred and full
     rank noise.
     %
