...
%
Of all possible causes (features of X), only a few account for Y, let them be represented as EX, with E a binary diagonal matrix: a diagonal of zeroes and ones, which correspond to active causal features of X.
%
...
%
We first predict X from Y, as in the inverse approach, and retrieve a robust least square estimate $\hat X$. Then, we calculate a robust least square estimator of $\hat X$ from X.
%
