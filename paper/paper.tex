% TODO: Talk about cause-effect methods, trace method, half-sibling
% TODO: Cite Roweis component analysis methods paper
...
To address this issue, we introduce ``Back-to-Back Regression'' (B2BR), a
method designed to measure, from a set of co-varying factors, the causal influences 
that most plausible account for multidimensional observations. After proving the consistency of B2BR,
we show that our method outperforms least-squares and related
...
identification and out-of-distribution prediction. Finally, we apply B2BR to
...
\begin{figure}[t!]
    ...
    \node[] (labY) at (\posY, 3){Observations};
    \node[] (labX) at (\posX, 3){Factors};
    \node[] (labE) at (\posE, 3.3){Cause};
    \node[] (labE) at (\posE, 3){selection};
    \node[] (labN) at (\posN, 3){Noise};
    \node[] (labF) at (\posF, 3.3){Cause-effect};
    \node[] (labF) at (\posF, 3){mapping};
...
%
Given the model in Equation~\eqref{eq:model}, \textbf{the goal} of Back-to-Back Regression (B2BR) is to estimate the matrix of causal influences $E$.
...
If using regularized least-squares, arguably the most commonly employed linear regression technique [CITE], the solution of B2BR has a closed form solution:
\begin{align}
...
%
To compensate for the reduction in sample size caused by the split, B2BR is repeated over many random splits, and the final estimate $\hat E$ of the causal influence matrix is the average over the estimates associated to each split [CITE BOOTSTRAP].
%
After obtaining $\hat{E}$, we can fit a final regression from $X \hat{E}$ to $Y$ as a final B2BR prediction rule.
...
Recall the rank factorisation property of the pseudo-inverse [CITE]: if $A=BC$ with $A\in\mathbb{R}^{m\times n}$ of rank $k$, $B\in\mathbb{R}^{m\times k}$ and $C\in\mathbb{R}^{k\times n}$, then $A^\dagger=C^\dagger B^\dagger$ and, since $B$ is full rank, $B^\dagger=(B^\top B)^{-1} B^\top$.
%
...
On the one hand, we have leveraged B2BR to obtain the estimate $\hat{E}$, which is a diagonal matrix with real entries.
%
...
In regression analysis, the traditional approach to this problem employs a $t$-test to check whether the regression coefficients differ from zero [CITE].
%
...
%
Letting $m_0$ and $m_1$ be the average values of the two clusters, $p$ and $d_x-p$ their size, and $v$ the total variance of the sample, we select the split maximizing the Sonquist-Morgan [CITE] criterion $\frac{p(d_x-p)}{d_x} \frac{(m_1 - m_0)^2}{v}$.
%
...
Back-to-back regression is closely related to Canonical Correlation Analysis [CITE].
%
