...
%
In particular, $\hat G = (Y^\top Y + \Lambda_Y)^{-1} Y^\top X$ and $\hat H=(X^\top X + \Lambda_X)^{-1} X^\top Y \hat G$, where $\Lambda_X$ and $\Lambda_Y$ are two diagonal matrices containing regularization parameters, useful to invert the covariance matrices of $X$ and $Y$ if these are ill-conditioned.
