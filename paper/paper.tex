...
As discussed in the introduction, our problem can be formulated (up to a redifinition of the features of X) as $Y=f(EX+N)$, with F an unknown function.
%
...
\begin{equation}
\begin{aligned}
arg \min_H \left \| k(FE)^{\dagger}Y - HX \right \|^2 &= arg \min_H \left \| k(FE)^{\dagger}F(EX+N) - HX \right \|^2 \\
&= arg \min_H \left \| (k(FE)^{\dagger}(FE)-H)X \right \| ^2 + \left \| k(FE)^{\dagger}FN \right \| ^2\\
&= arg \min_H \left \| (k(FE)^{\dagger}(FE)-H)X \right \| ^2\\
&=k(FE)^{\dagger}(FE)
\end{aligned}
\end{equation}
Thus, in the linear case, double regression retrieves $k(FE)^{\dagger}(FE)$. In many cases, if v s the variance of $FN$, k can be approximated as $k= {1\over{1+v}}$, it quantifies the amount of noise added to the causal process, and the difficulty of recovering the causal features.
