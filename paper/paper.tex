...
Finally, we apply B2B to magneto-encephalography and fMRI of 102 subjects
...
factors known to cause early visual and late associative cortical responses respectively. The results
...
\begin{figure}[t!]
  ...
  \node[] (reg1) at (6,-3.25) {$\hat{E} = \text{diag}(\underbrace{(X_2^\top X_2 + \Lambda_X)^{-1} X_2^\top Y_2\overbrace{(Y_1^\top Y_1 + \Lambda_Y)^{-1} Y_1^\top X_1}^{\text{1) } \hat{X} : \text{ regression from } Y \text{ to } X}}_{\text{2) regression from } X \text{ to } \hat{X}})$};
  \end{tikzpicture}
