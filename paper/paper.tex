...
\end{align}
%
where $\Lambda_X$ and $\Lambda_Y$ are two diagonal matrices containing regularization parameters, useful to invert the covariance matrices of $X$ and $Y$ if these are ill-conditioned.
...
\begin{align*}
    \arg \min_G \left \| YG - X \right \|^2 &= \arg \min_G \left \| (EX + N)FG \right\|^2 \\
...
%
Let $K \in \mathbb{R}^{d_x \times k}$ be the diagonal matrix obtained by deleting the right-most $d_x - k$ columns of $E$.
%
Then, $E = KK^\top$, and $XEF = (XK)(K^\top F)$, with both $XK$ and $K^\top$ of rank $k$.

Recall the rank factorisation property of the pseudo-inverse \citep{todo}: if $A=BC$ with $A\in\mathbb{R}^{m\times n}$ of rank $k$, $B\in\mathbb{R}^{m\times k}$ and $C\in\mathbb{R}^{k\times n}$, then $A^\dagger=C^\dagger B^\dagger$ and, since $C$ is full rank, $C^\dagger=C' (CC')^{-1}$.
%
...
In regression analysis, the traditional approach to this problem consists in testing whether the regression coefficients differ from zero.
%
...
%
Letting $m_0$ and $m_1$ be the average values of the two clusters, $p$ and $d_x-p$ their size, and $v$ the total variance of the sample, we select the split maximizing the Sonquist-Morgan \citep{todo} criterion $\frac{p(d_x-p)}{d_x} \frac{(m_1 - m_0)^2}{v}$.
%
...
Back-to-back regression is closely related to Canonical Correlation Analysis \citep[CCA]{todo}.
%
