...
\begin{figure}[t!]
  ...
  \caption{Average $\Delta R$ for each $Y$ dimension (Forward model), canonical components (PLS and CCA) or feature (B2B) across subjects. Formally, Backward model cannot have a $\Delta R$ because it never demix multiple $X$ features. In such cases, we thus simply report the decoding score.}
  \label{fig:meg_supp}
\end{figure}
